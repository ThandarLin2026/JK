...
# Fixed: Script run errors, Today Sales, Ping color, THB, Flow tracking
# Fix: New accounts sync to ZIVPN auth.config so they can connect
# Supports: x86_64 / amd64 VPS
...
if [[ "$ARCH" != "x86_64" && "$ARCH" != "amd64" ]]; then
    echo -e "${RED}Error: This installer supports x86_64 / amd64 only.${NC}"
...
if [ ! -f /etc/zivpn/zivpn.key ] || [ ! -s /etc/zivpn/zivpn.key ] || [ ! -f /etc/zivpn/zivpn.crt ] || [ ! -s /etc/zivpn/zivpn.crt ]; then
...
# Normalize any old config format to the expected Zivpn format
python3 - <<'PY'
import json
...
# Migrate old top-level config array into auth.config
if "auth" not in data or not isinstance(data.get("auth"), dict):
    data["auth"] = {"mode": "passwords", "config": []}

if "config" in data and isinstance(data.get("config"), list) and not data["auth"].get("config"):
    data["auth"]["config"] = data["config"]

data.pop("config", None)

data["listen"] = data.get("listen") or ":5667"
data["cert"] = data.get("cert") or "/etc/zivpn/zivpn.crt"
data["key"] = data.get("key") or "/etc/zivpn/zivpn.key"
data["obfs"] = data.get("obfs") or "zivpn"
data["auth"]["mode"] = "passwords"
if not isinstance(data["auth"].get("config"), list):
    data["auth"]["config"] = ["zi"] if default else []

if not data["auth"]["config"]:
    data["auth"]["config"] = ["zi"]
...
HOST_NAME="$host_name" IP_ADDR="$ip_addr" python3 - <<'PY'
from pathlib import Path
import os

p = Path("/etc/zivpn/index.html")
text = p.read_text(encoding="utf-8")
text = text.replace("__HOST_NAME__", os.environ["HOST_NAME"])
text = text.replace("__IP_ADDR__", os.environ["IP_ADDR"])
...
function loadZivpnConfig() {
    let cfg = safeReadJSON(CONFIG_PATH, defaultZivpnConfig());
    if (!cfg || typeof cfg !== 'object' || Array.isArray(cfg)) {
        cfg = defaultZivpnConfig();
    }

    if (!cfg.auth || typeof cfg.auth !== 'object' || Array.isArray(cfg.auth)) {
        cfg.auth = { mode: 'passwords', config: [] };
    }

    if (Array.isArray(cfg.config) && (!Array.isArray(cfg.auth.config) || cfg.auth.config.length === 0)) {
        cfg.auth.config = cfg.config;
    }

    cfg.listen = cfg.listen || ':5667';
    cfg.cert = cfg.cert || '/etc/zivpn/zivpn.crt';
    cfg.key = cfg.key || '/etc/zivpn/zivpn.key';
    cfg.obfs = cfg.obfs || 'zivpn';
    cfg.auth.mode = 'passwords';
    if (!Array.isArray(cfg.auth.config)) cfg.auth.config = [];

    return cfg;
}

function saveZivpnConfigPasswords(passwords) {
    const cfg = loadZivpnConfig();
    cfg.auth.mode = 'passwords';
    cfg.auth.config = Array.from(passwords || []).map(v => String(v));

    // Keep compatibility for any older tooling that still reads top-level config
    cfg.config = [...cfg.auth.config];
...
function syncConfigPasswords(users) {
    const passwords = users.map(u => u.password).filter(v => String(v || '').length > 0);
    saveZivpnConfigPasswords(passwords);
}
...
# Match the upstream UDP forwarding setup so client traffic can reach the server
sysctl -w net.core.rmem_max=16777216 >/dev/null 2>&1 || true
sysctl -w net.core.wmem_max=16777216 >/dev/null 2>&1 || true
iptables -t nat -C PREROUTING -i "$(ip -4 route ls | grep default | grep -Po '(?<=dev )(\S+)' | head -1)" -p udp --dport 6000:19999 -j DNAT --to-destination :5667 2>/dev/null || \
