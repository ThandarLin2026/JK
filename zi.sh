...
if [ ! -s /etc/zivpn/data/users.json ]; then
                ...
                const card = \`
                    <div class="glass p-6 rounded-[2rem] border-t-4 \${u.status==='online'?'border-green-500':'border-blue-500'} relative shadow-lg">
                        ...
                        <div class="flex justify-between items-center pt-2">
                            ...
                            <div class="flex gap-2">
                                <button class="p-2 hover:bg-yellow-500/20 rounded-lg text-yellow-400" onclick="alert('Edit not configured yet')"><i data-lucide="edit-3" class="w-5 h-5"></i></button>
        ...
        try {
            ...
            // Add User Account
                ...
                try {
                    ...
                    if (!Array.isArray(config.config)) config.config = [];
            ...
            // Delete User
                ...
                try {
                    ...
                    if (user) {
                        let config = readJson(CONFIG_PATH, { config: [] });
                        if (!Array.isArray(config.config)) config.config = [];
            ...
            // Restore
                ...
                try {
                    ...
                    if (!Array.isArray(restoredUsers)) {
                        res.writeHead(400, { 'Content-Type': 'text/plain' });
                        res.end('Restore file must be an array');
                        return;
                    }
                    ...
                    const config = readJson(CONFIG_PATH, { config: [] });
                    config.config = restoredUsers.map(u => u.password).filter(Boolean);
                    writeJson(CONFIG_PATH, config);
...
echo -e "${YELLOW}Fix Applied: better validation + safer JSON handling + stable create account API.${NC}"
echo -e "${GREEN}===============================================${NC}"
