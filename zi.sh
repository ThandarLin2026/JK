                ...
                const card = \`
                    <div class="glass p-6 rounded-[2rem] border-t-4 \${u.status==='online'?'border-green-500':'border-blue-500'} relative shadow-lg">
                        ...
                        <div class="flex justify-between items-center pt-2">
                            ...
                            <div class="flex gap-2">
                                <button class="p-2 hover:bg-yellow-500/20 rounded-lg text-yellow-400"><i data-lucide="edit-3" class="w-5 h-5"></i></button>
    ...
    req.on('end', () => {
        ...
        // Add User Account
            ...
            try {
                ...
                if(!username || !password) {
                    res.writeHead(400); res.end('Missing Fields');
                    return;
                }
        ...
        // Restore
            ...
            try {
                ...
                let config = JSON.parse(fs.readFileSync(CONFIG_PATH));
                config.config = restoredUsers.map(u => u.password);
                fs.writeFileSync(CONFIG_PATH, JSON.stringify(config, null, 2));
...
echo -e "${YELLOW}Fix Applied: JSON Content-Type added to requests.${NC}"
echo -e "${GREEN}===============================================${NC}"
