...
# Fixed account creation + safer JSON handling + better panel stability
# Optimized for x86_64
# Changes:
# - Keep ZIVPN config in upstream format: auth.mode=passwords and auth.config array
# - Generate cert files
# - Show password on user cards
# - Add copy buttons on user cards and modal
# - Preserve the rest of the panel behavior
...
# --- ZIVPN Config (upstream-compatible) ---
...
<body class="bg-[#0b0f1a] text-gray-100 min-h-screen font-sans">
    ...
    <script>
        let users = [];
        const host = __HOST_NAME_JSON__;
        const ip = __IP_ADDR_JSON__;
...
txt = txt.replace("__HOST_NAME_JSON__", json.dumps(os.environ["HOST_NAME"]))
txt = txt.replace("__IP_ADDR_JSON__", json.dumps(os.environ["IP_ADDR"]))
...
function ensureFiles() {
    ...
    if (!fs.existsSync(CONFIG_PATH) || fs.readFileSync(CONFIG_PATH, 'utf8').trim() === '') {
        ...
        // Upgrade old config format if needed
...
function setPasswordList(passwords) {
    ...
    cfg.auth = cfg.auth || {};
    cfg.auth.mode = 'passwords';
    cfg.auth.config = [...new Set(passwords.map(v => String(v).trim()).filter(Boolean))];
    if (cfg.auth.config.length === 0) cfg.auth.config = ['zi'];
    delete cfg.config;
...
function getPasswordList() {
    const cfg = getConfig();
    return Array.isArray(cfg.auth?.config) ? cfg.auth.config : [];
}
...
echo -e "${YELLOW}Fix Applied: upstream-compatible auth.config + password display + copy buttons.${NC}"
echo -e "${GREEN}===============================================${NC}"
