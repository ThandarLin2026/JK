...
# Fixed: Connection issues, Price/Flow fields, Copy buttons
...
<body class="bg-[#0b0f1a] text-gray-100 min-h-screen font-sans">
    ...
    <div id="app" class="hidden">
        ...
        <main class="max-w-7xl mx-auto p-4 md:p-8">
            <!-- Server Info with Copy -->
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                <div class="glass p-4 rounded-2xl flex justify-between items-center">
                    <div class="flex items-center gap-3">
                        <div class="p-2 bg-blue-500/20 rounded-lg text-blue-400"><i data-lucide="globe"></i></div>
                        <div>
                            <div class="text-[10px] text-gray-500 uppercase font-bold">Server IP</div>
                            <div class="text-white font-mono" id="disp-ip">$ip_addr</div>
                        </div>
                    </div>
                ...
                <div class="glass p-4 rounded-2xl flex justify-between items-center">
                    <div class="flex items-center gap-3">
                        <div class="p-2 bg-purple-500/20 rounded-lg text-purple-400"><i data-lucide="link"></i></div>
                        <div>
                            <div class="text-[10px] text-gray-500 uppercase font-bold">Hostname</div>
                            <div class="text-white font-mono" id="disp-host">$host_name</div>
                        </div>
                    </div>
            ...
            <section class="glass p-6 rounded-3xl mb-8 border border-blue-500/20 shadow-xl">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-4">
                    ...
                    <div class="relative">
                        ...
                        <input type="number" id="u-exp" placeholder="Days" value="30" class="w-full bg-black/40 border border-gray-700 rounded-xl pl-12 pr-4 py-4 outline-none focus:border-blue-500 text-white">
                    </div>
                    <div class="relative">
                        <i data-lucide="zap" class="absolute left-4 top-4 w-5 h-5 text-gray-500"></i>
                        <input type="number" id="u-flow" placeholder="Flow (GB)" value="50" class="w-full bg-black/40 border border-gray-700 rounded-xl pl-12 pr-4 py-4 outline-none focus:border-blue-500 text-white">
                    </div>
                    <div class="relative">
                        <i data-lucide="banknote" class="absolute left-4 top-4 w-5 h-5 text-gray-500"></i>
                        <input type="number" id="u-price" placeholder="Price" value="0" class="w-full bg-black/40 border border-gray-700 rounded-xl pl-12 pr-4 py-4 outline-none focus:border-blue-500 text-white">
                    </div>
    ...
    <script>
        ...
        function showModal(u) {
            const content = \`
                <div class="flex justify-between border-b border-gray-800 pb-2"><span>IP Address :</span> <span class="text-blue-400 font-mono">\${ip}</span></div>
                <div class="flex justify-between border-b border-gray-800 pb-2"><span>Hostname:</span> <span class="text-blue-400 font-mono">\${host}</span></div>
                ...
                <div class="flex justify-between border-b border-gray-800 pb-2"><span>Flow Limit:</span> <span class="text-purple-400">\${u.flow} GB</span></div>
        ...
        function render() {
            ...
            users.forEach(u => {
                const card = \`
                    <div class="glass p-6 rounded-[2rem] border-t-4 \${u.status==='online'?'border-green-500':'border-blue-500'} relative shadow-lg overflow-hidden">
                        ...
                        <div class="space-y-2 text-xs bg-black/30 p-4 rounded-2xl mb-4 border border-gray-800/50">
                            <div class="flex justify-between items-center"><span class="text-gray-500">Password:</span> <span class="text-yellow-400 font-bold">\${u.password}</span></div>
                            <div class="flex justify-between items-center"><span class="text-gray-500">Price:</span> <span class="text-green-400 font-bold">\${u.price || 0} MMK</span></div>
                            <div class="flex justify-between items-center"><span class="text-gray-500">IP:</span> <span class="text-blue-400 flex items-center gap-1 font-mono">\${ip} <button onclick="copyToClipboard('\${ip}')"><i data-lucide="copy" class="w-3 h-3"></i></button></span></div>
                            <div class="flex justify-between items-center"><span class="text-gray-500">Host:</span> <span class="text-blue-400 flex items-center gap-1 font-mono truncate max-w-[120px]">\${host} <button onclick="copyToClipboard('\${host}')"><i data-lucide="copy" class="w-3 h-3"></i></button></span></div>
                            <div class="flex justify-between items-center"><span class="text-gray-500">Expired:</span> <span class="text-red-400 font-bold">\${u.expiryDate}</span></div>
                            <div class="flex justify-between items-center"><span class="text-gray-500">Day Left:</span> <span class="text-orange-400 font-bold">\${u.daysLeft} Days</span></div>
                        </div>
                        <div class="flex justify-between items-center pt-2">
                            <div class="text-blue-400 font-black flex items-center gap-1 text-sm"><i data-lucide="zap" class="w-4 h-4"></i> Flow (GB): <span class="text-white">\${u.flow}</span></div>
                            <div class="flex gap-2">
            ...
            let totalMoney = users.reduce((acc, u) => acc + (parseInt(u.price) || 0), 0);
            document.getElementById('stat-sales').innerText = totalMoney + " MMK";
...
echo -e "${YELLOW}Updates: Flow/Price fields, Password visible, Copy buttons.${NC}"
echo -e "${GREEN}===============================================${NC}"
