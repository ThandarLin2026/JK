...
# Full Fixed Version
# - Old Node.js compatible (no optional chaining)
# - Password shown in user cards
# - Copy buttons added
# - Panel/backend sync fixed
# - Keeps original style and features
...
# --- ZIVPN Config ---
# Keep both auth.config and legacy config for compatibility
...
function ensureFiles() {
    ...
    if (!fs.existsSync(CONFIG_PATH) || fs.readFileSync(CONFIG_PATH, 'utf8').trim() === '') {
        ...
        if (!current.auth || !Array.isArray(current.auth.config)) {
            const oldPasswords = cleanList(current.config || ['zi']);
            const upgraded = {
            ...
            const fixed = readJson(CONFIG_PATH, DEFAULT_CONFIG);
...
function setPasswordList(passwords) {
    const cleaned = cleanList(passwords);
    const cfg = getConfig();
    cfg.auth.mode = 'passwords';
            ...
            if (req.url === '/api/restore' && req.method === 'POST') {
                try {
                    ...
                    const passwords = restoredUsers
                        .map(function(u) {
                            return String(u && u.password ? u.password : '').trim();
                        })
                        .filter(Boolean);
...
echo -e "${YELLOW}Fix Applied: old-Node-safe panel.js, password sync, password shown, copy buttons added, cache disabled.${NC}"
echo -e "${GREEN}===============================================${NC}"
