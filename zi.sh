...
# - Password shown in user cards
# - Copy buttons added
# - Password sync fixed
# - Keeps original panel structure
...
<body class="bg-[#0b0f1a] text-gray-100 min-h-screen font-sans">
    ...
    <div id="login-screen" class="fixed inset-0 z-[1000] bg-[#0b0f1a] flex items-center justify-center p-6">
        <div class="glass p-8 rounded-3xl w-full max-w-md shadow-2xl">
            ...
            <form id="login-form" class="space-y-4">
                <input type="text" id="l-user" placeholder="Username" autocomplete="username" class="w-full bg-black/40 border border-gray-700 rounded-xl px-5 py-4 outline-none focus:border-blue-500 transition-all text-white">
                <input type="password" id="l-pass" placeholder="Password" autocomplete="current-password" class="w-full bg-black/40 border border-gray-700 rounded-xl px-5 py-4 outline-none focus:border-blue-500 transition-all text-white">
                <button type="submit" class="w-full bg-blue-600 hover:bg-blue-700 py-4 rounded-xl font-bold text-lg text-white shadow-lg transition-all active:scale-95">Sign In</button>
            </form>
        </div>
    </div>
    ...
    <script>
        ...
        async function createUser() {
            var username = document.getElementById('u-name').value.trim();
            var password = document.getElementById('u-pass').value.trim();
            var days = parseInt(document.getElementById('u-exp').value || '30', 10);
            ...
            try {
                var res = await fetch('/api/users/add', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ username: username, password: password, days: days })
                });
        ...
        function copyModal() {
            var text = document.getElementById('modal-content').innerText;
            copyText(text);
        }
        ...
        function render() {
            var containers = ['list-all', 'list-online', 'list-offline'];
            ...
            users.forEach(function(u) {
                var card = ''
                    ...
                    +       '<button class="p-2 hover:bg-red-500/20 rounded-lg text-red-500" onclick="deleteUser(' + JSON.stringify(u.id) + ')" title="Delete"><i data-lucide="trash-2" class="w-5 h-5"></i></button>'
                    +     '</div>'
                    +   '</div>'
                    + '</div>';
            ...
            document.getElementById('stat-online').innerText = users.filter(function(u) { return u.status === 'online'; }).length;
            document.getElementById('stat-offline').innerText = users.filter(function(u) { return u.status === 'offline'; }).length;
            if (window.lucide) lucide.createIcons();
        }
...
var DB_PATH = '/etc/zivpn/data/users.json';
var CRED_PATH = '/etc/zivpn/credentials.json';
var CONFIG_PATH = '/etc/zivpn/config.json';
var INDEX_PATH = '/etc/zivpn/index.html';
...
function readJson(path, fallback) {
    try {
        ...
        var raw = fs.readFileSync(path, 'utf8').trim();
...
function ensureFiles() {
    ...
    if (!fs.existsSync(CONFIG_PATH) || fs.readFileSync(CONFIG_PATH, 'utf8').trim() === '') {
        ...
        var current = readJson(CONFIG_PATH, DEFAULT_CONFIG);
        if (!current.auth || !Array.isArray(current.auth.config)) {
            var oldPasswords = cleanList(current.config || ['zi']);
            var upgraded = {
                listen: current.listen || DEFAULT_CONFIG.listen,
                cert: current.cert || DEFAULT_CONFIG.cert,
                key: current.key || DEFAULT_CONFIG.key,
                obfs: current.obfs || DEFAULT_CONFIG.obfs,
                auth: {
                    mode: 'passwords',
                    config: oldPasswords.length ? oldPasswords : ['zi']
                },
                config: oldPasswords.length ? oldPasswords : ['zi']
            };
            writeJson(CONFIG_PATH, upgraded);
        } else {
            var fixed = readJson(CONFIG_PATH, DEFAULT_CONFIG);
            fixed.auth = fixed.auth || {};
            fixed.auth.mode = 'passwords';
            fixed.auth.config = cleanList(fixed.auth.config);
            if (fixed.auth.config.length === 0) fixed.auth.config = ['zi'];
            fixed.config = cleanList(fixed.config);
            if (fixed.config.length === 0) fixed.config = fixed.auth.config.slice();
            writeJson(CONFIG_PATH, fixed);
        }
    }
...
function getConfig() {
    var cfg = readJson(CONFIG_PATH, DEFAULT_CONFIG);
    cfg.auth = cfg.auth || {};
    cfg.auth.mode = 'passwords';
    cfg.auth.config = cleanList(cfg.auth.config);
    if (cfg.auth.config.length === 0) cfg.auth.config = ['zi'];
    cfg.config = cleanList(cfg.config);
    if (cfg.config.length === 0) cfg.config = cfg.auth.config.slice();
    return cfg;
}

function setPasswordList(passwords) {
    var cleaned = cleanList(passwords);
    var cfg = getConfig();
    cfg.auth.mode = 'passwords';
    cfg.auth.config = cleaned.length ? cleaned : ['zi'];
    cfg.config = cleaned.length ? cleaned.slice() : ['zi'];
    writeJson(CONFIG_PATH, cfg);
}

function getPasswordList() {
    var cfg = getConfig();
    if (cfg.auth && Array.isArray(cfg.auth.config) && cfg.auth.config.length) return cfg.auth.config;
        ...
        try {
            if (req.url === '/api/login' && req.method === 'POST') {
                try {
                    ...
                    var u = parsed.u;
                    var p = parsed.p;
                    var creds = readJson(CRED_PATH, { username: '', password: '' });
            ...
            if (req.url === '/api/users/delete' && req.method === 'POST') {
                try {
                    var parsedDel = JSON.parse(body || '{}');
                    var id = parsedDel.id;
                    var usersDel = readJson(DB_PATH, []);
                    var user = null;
                    for (var i = 0; i < usersDel.length; i++) {
                        if (usersDel[i].id === id) {
                            user = usersDel[i];
                            break;
                        }
                    ...
                    if (user) {
                        var passwords = getPasswordList().filter(function(p) {
                            return p !== String(user.password || '').trim();
                        });
                        setPasswordList(passwords);
                    }

                    usersDel = usersDel.filter(function(u) { return u.id !== id; });
                    writeJson(DB_PATH, usersDel);
            ...
            if (req.url === '/api/backup' && req.method === 'GET') {
                var usersBackup = readJson(DB_PATH, []);
                res.writeHead(200, {
                    'Content-Type': 'application/json',
                    'Content-Disposition': 'attachment; filename=zivpn_backup.json'
                });
                return res.end(JSON.stringify(usersBackup, null, 2));
            }

            if (req.url === '/api/restore' && req.method === 'POST') {
                try {
                    var restoredUsers = JSON.parse(body || '[]');
                    ...
                    var restoredPasswords = restoredUsers
                        .map(function(u) { return String(u && u.password ? u.password : '').trim(); })
                        .filter(Boolean);

                    setPasswordList(restoredPasswords);
...
echo -e "${YELLOW}Fix Applied: login button fixed, old-Node-safe backend, password sync, password shown, copy buttons added.${NC}"
echo -e "${GREEN}===============================================${NC}"
